...
const modalServicio = new bootstrap.Modal(
  document.getElementById("modalAdminServicios")
);
...
const crearServicio = () => {
  estoyCreando = true;
  if (
    validarCantidadCaracteres(nombreServicio, 3, 30) === true &&
    validarCantidadCaracteres(descripcionServicio, 10, 100) &&
    validarUrl(imagenServicio)
  ) {
...
window.borrarServicio = (id) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      //logica para borrar
      const posicionServicioBuscado = listaServicios.findIndex(
...
const modificarServicio = () => {
  //Posicion
  const obtenerServicio = listaServicios.findIndex(
    (res) => res.id === servicioAEditar.id
  );
  //actualizar datos del array
  listaServicios[obtenerServicio].nombreServicio = nombreServicio.value;
  listaServicios[obtenerServicio].descripcionServicio =
    descripcionServicio.value;
